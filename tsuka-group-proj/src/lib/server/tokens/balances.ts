...
export async function getTokensWithPotentialBalance(walletAddress: string): Promise<Array<string>> {
    ...
    for(let address of pairAddresses) {
        ...
        const tokenPairInfo = await getTokenNamesFromPair("0x28cee28a7c4b4022ac92685c07d2f33ab1a0e122")
        if(tokenPairInfo.success) {
            ...
            if(pairedAddress && !tokenAddresses.includes(pairedAddress) && baseAddress != "-") tokenAddresses.push(pairedAddress)
        }
    }

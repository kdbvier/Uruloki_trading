...
import {
  ...
  transformData,
} from "../bitquery/getBitqueryStreamData";
...
export const fetchData = async (
  ...
  setCandleStickTime: any,
  setFirstBitquery: any,
  setForwardTime: any,
  setDatas: any,
  time: number = 15
) => {
...
export const addMarkers = (
  activeOrdersByTokenpair: Array<Order>,
...
export const updateChartSize = (chartRef: HTMLDivElement, chart: IChartApi) => {
  ...
  new ResizeObserver((entries) => {
    if (entries.length === 0 || entries[0].target !== chartRef) {
      return;
    }

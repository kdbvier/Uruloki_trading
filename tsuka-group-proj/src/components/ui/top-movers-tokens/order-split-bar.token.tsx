...
export const OrderSplitBar: React.FC<OrderSplitBarProps> = ({
  ...
  return (
    <div className="w-full">
      <div className="w-full rounded-md h-1.5 bg-red text-end text-tsuka-700 font-medium flex items-center">
        <div
      ...
      <div className="w-full mt-2 text-[12px] leading-4 flex items-stretch">
        <div className="w-full text-start"><span className="text-tsuka-100">Buy : </span><span className="text-green">{buyOrderCount}</span></div>
        <div className="w-full text-end"><span className="text-tsuka-100">Sell : </span><span className="text-red">{sellOrderCount}</span></div>
      </div>
    </div>
  );
};
